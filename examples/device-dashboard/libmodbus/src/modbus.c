...
static int read_registers(modbus_t *ctx, int function, int addr, int nb, uint16_t *dest)
{
    ...
    if (rc > 0) {
        ...
        rc = _modbus_receive_msg(ctx, rsp, MSG_CONFIRMATION);
        if (rc == -1){
            //syslog(LOG_INFO, "%s %d _modbus_receive_msg fail:%d",__func__, ctx->s ,rc);
            return -1;
        }
            
        rc = check_confirmation(ctx, req, rsp, rc);
        if (rc == -1){
            //syslog(LOG_INFO, "%s %d check_confirmation fail:%d",__func__, ctx->s, rc);
            return -1;
        }
        ...
        //syslog(LOG_INFO, "%s %d send_msg fail:%d",__func__, ctx->s, rc);
    }
